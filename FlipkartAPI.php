...
/**
 * Class ShopifyAPI used to connect ot any shopify app
 */
class FlipkartAPI
{
	private $_API = array();
	private static $_KEYS = array('API_KEY','API_SECRET','API_URL','ACCESS_TOKEN');
	...
	{
		$data = $this->call(['METHOD' => 'GET', 'URL' => 'https://' . $this->_API['API_URL'] . '/oauth-service/oauth/token?grant_type=client_credentials&scope=Seller_Api'], FALSE);
		if(is_array($data)){
			return false;
		} else {
			$this->_API['ACCESS_TOKEN']= $data->access_token;
			return true;
		}
	}
    ...
    public function call($userData = array(), $verifyData = TRUE)
    {
	    ...
	    if (array_key_exists('ACCESS_TOKEN', $this->_API))
	    {
		    $defaultHeaders[] = 'Authorization:Bearer ' . $this->_API['ACCESS_TOKEN'];
			 ...
			 $defaultHeaders[] ='Authorization:Basic ' .base64_encode("".$this->_API['API_KEY'].":".$this->_API['API_SECRET']."");
		}
